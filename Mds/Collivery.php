...
class Collivery {
	...
	function __construct( array $config = [], $cache = null )
	{
		...
		$this->config = (object) [
			...
			'app_host'      => 'Joomla - Virtuemart', // Framework/CMS name and version, eg 'Wordpress 3.8.1 WooCommerce 2.0.20' / 'Joomla! 2.5.17 VirtueMart 2.0.26d'
	...
	protected function authenticate()
	{
		...
		if (
            ...
            $this->default_address_town_id = $authCache['client']['primary_address']['town_id'];
            $this->default_address_suburb_id = $authCache['client']['primary_address']['suburb_id'];
            $this->default_address_location_type_id = $authCache['client']['primary_address']['location_type']['id'];
     ...
     * @throws Exception
     */
    public function getSuburbs($town_id = -1)
    {
...
/*                error_log(print_r($result));*/
    ...
    {
        $location_types = $this->make_key_value_array($this->getLocationTypes());
        $towns = $this->make_key_value_array($this->getTowns());
        $suburbs = $this->make_key_value_array($this->getSuburbs($data['town_id']));
